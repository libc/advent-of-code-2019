...
fn main() -> std::io::Result<()> {
    println!("{}", solve("inputs/day18-0.txt")?);
    println!("{}", solve("inputs/day18-1.txt")?);
    println!("{}", solve("inputs/day18-2.txt")?);
    println!("{}", solve("inputs/day18-3.txt")?);
    println!("{}", solve("inputs/day18-4.txt")?);
    println!("{}", solve("inputs/day18.txt")?);
    Ok(())
}
...
fn solve(file: &str) -> std::io::Result<usize> {
    ...
    while !queue.is_empty() {
        ...
        let accessible_keys = find_reachable_keys(&puzzle, &item.collected_keys, item.pos);
...
fn find_start_pos(puzzle: &Vec<Vec<Tile>>) -> (i64, i64) {
    for (y, l) in puzzle.iter().enumerate() {
        for (x, t) in l.iter().enumerate() {
            match t {
                Tile::StartPosition => return (x as i64, y as i64),
                _ => continue,
            }
        }
    }
