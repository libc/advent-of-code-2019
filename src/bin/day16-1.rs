use std::iter::repeat;

fn apply_phase(input: Vec<i64>) -> Vec<i64> {
    (0..input.len())
        .map(|idx| {
            let pattern = repeat(0)
                .take(idx + 1)
                .chain(repeat(1).take(idx + 1))
                .chain(repeat(0).take(idx + 1))
                .chain(repeat(-1).take(idx + 1))
                .cycle()
                .skip(1);
            input
                .iter()
                .zip(pattern)
                .map(|(a, b)| a * b)
                .sum::<i64>()
                .abs()
                % 10
        })
        .collect()
}
fn phases(input: &str, n: usize) -> String {
    let mut v = input
        .chars()
        .map(|c| c.to_digit(10).unwrap() as i64)
        .collect::<Vec<i64>>();

    for i in 0..n {
        v = apply_phase(v);
    }

    v.iter()
        .map(|d| format!("{}", d))
        .collect::<Vec<String>>()
        .join("")
}

fn phases_skip(input: &str, n: usize) -> String {
    let output = phases(
        &repeat(input)
            .map(|s| String::from(s))
            .take(10000)
            .collect::<Vec<String>>()
            .join(""),
        n,
    );

    let skip = output.get(0..7).unwrap().parse::<usize>().unwrap();

    String::from(output.clone().get(skip..(skip + 8)).unwrap())
}

fn main() {
    println!("{}", phases("12345678", 1));
    println!("{}", phases("12345678", 2));
    println!("{}", phases("12345678", 3));
    println!("{}", phases("12345678", 4));
    println!(
        "{}",
        phases("80871224585914546619083218645595", 100)
            .get(0..8)
            .unwrap()
    );
    println!(
        "{}",
        phases("19617804207202209144916044189917", 100)
            .get(0..8)
            .unwrap()
    );
    println!(
        "{}",
        phases("69317163492948606335995924319873", 100)
            .get(0..8)
            .unwrap()
    );

    let task = "";
    println!("{}", phases(task, 100).get(0..8).unwrap());

    println!("");
    println!("part 2:");

    let task = "";
    println!("{}", phases_skip(task, 100));

    println!("{}", phases_skip("80871224585914546619083218645595", 100));
    println!("{}", phases_skip("19617804207202209144916044189917", 100));
    println!("{}", phases_skip("69317163492948606335995924319873", 100));

}
