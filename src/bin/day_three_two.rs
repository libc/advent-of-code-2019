...
fn parse_action(s: &str) -> (i64, i64, i64) {
    let n = s.get(1..).unwrap().parse::<i64>().unwrap();
...
fn trace(path: &String) -> i64 {
    let mut grid: HashMap<(i64, i64), HashSet<i64>> = HashMap::new();
    let mut steps: HashMap<(i64, i64), HashMap<i64, i64>> = HashMap::new();
    ...
    for wire in path.split("\n") {
        ...
        for action in wire.split(",") {
            let (dx, dy, n) = parse_action(action);
            ...
            for _ in 0..n {
                ...
                let o = grid.get(&p);
                if o.is_some() {
                    if !o.unwrap().is_empty() {
                        let s = o.unwrap();
                        if s.len() > 1 || s.len() == 1 && !s.contains(&w) {
                            intersections.insert(p);
                        }
                    }
                } else {
                    grid.insert((x, y), HashSet::new());
                }

                if steps.get(&(x, y)).is_none() {
                    steps.insert((x, y), HashMap::new());
                }

                if !steps.get(&(x, y)).unwrap().get(&w).is_some() {
                    steps.get_mut(&(x, y)).unwrap().insert(w, step);
                }

                grid.get_mut(&(x, y)).unwrap().insert(w);
            }
        }
    }
    ...
    intersections
        .into_iter()
        .map(|(x, y)| steps.get(&(x, y)).unwrap().iter().map(|(_, v)| v).sum())
        .min()
        .unwrap()
}
...
fn main() {
    ...
    for p in paths {
        println!("distance: {}", trace(&p.to_owned()))
    }
}
